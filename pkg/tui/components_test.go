...
func TestSelectList_Render(t *testing.T) {
	items := []SelectItem{
		{Value: "1", Label: "First", Description: "The first item"},
		{Value: "2", Label: "Second", Description: "The second item"},
		{Value: "3", Label: "Third", Description: "The third item"},
	}

	sl := NewSelectList(items, 5)
	lines := sl.Render(80)
...
func TestSelectList_Navigation(t *testing.T) {
	items := []SelectItem{
		{Value: "1", Label: "First"},
	...
	sl.HandleInput("\x1b[B") // down
	sl.HandleInput("\x1b[B") // down (should stop at last)
...
func TestSelectList_Filter(t *testing.T) {
	...
	lines := sl.Render(80)
	if len(lines) != 1 {
		t.Fatalf("expected 1 filtered item, got %d", len(lines))
	}
}

func TestSelectList_EmptyFilter(t *testing.T) {
	items := []SelectItem{
		{Value: "apple", Label: "Apple"},
	}

	sl := NewSelectList(items, 5)
	sl.SetFilter("xyz") // No match

	lines := sl.Render(80)
	if len(lines) == 0 {
		t.Fatal("expected at least 1 line (no match message)")
	}
}

func TestSelectList_NoItems(t *testing.T) {
	sl := NewSelectList(nil, 5)
	lines := sl.Render(80)
	if len(lines) == 0 {
		t.Fatal("expected at least 1 line (no items message)")
	}
}
